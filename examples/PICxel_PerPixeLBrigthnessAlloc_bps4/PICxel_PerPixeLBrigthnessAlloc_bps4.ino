 ...
 * with a single color, and then initalize the per-pixel brightness values
 * to a ramp, starting with 0 brightness for pixel 0, and 100% brightness
 * for pixel NUMBER_OF_LEDS.
 * 
 * Then in loop(), we just increment each pixe's brightness and update the
...
#define LED_PIN 18                // Use pin 18 for RGB Station board, change for your board
